...
// token datv abase mai save
   user.emailVerificationToken = hashedToken
   user.emailVerificationExpiry = tokenExpiry

   await user.save({validateBeforeSave: false})
// verification email send karna +
   await sendEmail({
     email: user?.email,
...
const refreshAccessToken = asyncHandler(async(req,res)=>{
 ...
 try {
  ...
  const user =   await User.findById(decodedToken?._id)
   ...
   const {accessToken, refreshToken: newRefresToken} = await generateAccessAndRefreshTokens(user._id)
   ...
   return res.status(200)
   .cookie("accessToken", accessToken, options)
   .cookie("refeshTOKEN", newRefresToken, options)
   .json(
      new ApiResponse(
         200,
         {
...
const forgetPassword = asyncHandler(async(req, res)=>{
 ...
 .json(
   new ApiResponse(
      200,
      {},
      "Password reset mail has been sent on mail id"
   )
 )
})
